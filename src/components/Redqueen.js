  ...
  useEffect(() => {
    ...
    const adjustBackgroundPlayback = () => {
      if (redQueen_alice.getAnimation().playbackRate < 0.8) {
        ...
        sceneries.forEach(function (animate) {
          animate.playbackRate =
            redQueen_alice?.getAnimation()?.playbackRate * 2.5
        })
      }
    }

    setInterval(() => {
      if (redQueen_alice.getAnimation().playbackrateRQ > 0.4) {
    ...
    document.addEventListener('click', () => {
      redQueen_alice
        .getAnimation()
        .updatePlaybackRate(redQueen_alice.getAnimation().playbackRate * 1.1)
      adjustBackgroundPlayback()
    })
  })
